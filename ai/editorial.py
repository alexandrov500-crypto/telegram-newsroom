    ...
    *,
    ...
    blocks.extend(
        [
            ...
            "* единый шаблон wire-канала: headline отдельно (json) + полноценный post;",
            "* headline: одна строка — главный факт с цифрой/решением, без кликбейта и без «…»;",
            f"* post: 3–5 тезисов с маркером ▸ (по одному факту на строку), {lo}–{hi} символов;",
            "* не дублируй headline в первом тезисе; без таймкодов ТВ и без «Главные новости — в утреннем выпуске»;",
            "* структура post: что произошло → контекст/механизм → завершённый итог одним предложением;",
            "* последнее предложение — законченный вывод для читателя, не обрыв и не «продолжение следует»;",
            ...
            "* не обрывай мысль на «…» — доведи предложение до конца;",
            "* без вопросов читателю, без «продолжение темы», без призывов подписаться;",
            "* визуальная чистота: короткие абзацы, легко сканируемый текст;",
        ]
    )
    ...
    blocks.extend(
        [
            ...
            f"* до {hi} символов в поле post (если задан отдельный заголовок — он не входит в этот лимит);",
...
def _wire_post_char_range() -> tuple[int, int]:
    try:
        from app.editorial.wire_post_format import ai_post_char_range

        return ai_post_char_range()
    except Exception:
        return 520, 980
...
def _style_block_ru(style: str) -> str:
    ...
    m = {
        ...
        "cb-economics-brief": (
            "Стиль: как у эталонных wire-каналов (@cb_economics, @thebell_io, @rbc_news) — единый формат каждого поста. "
            "headline: одна строка, главный факт (цифра/решение/событие), без интриги и без вопроса. "
            f"post: 3–5 тезисов с маркером ▸ (один факт на строку, {lo}–{hi} символов). "
            "Структура: факт → контекст → вывод. Без таймкодов, без дублирования заголовка. "
            "Последнее предложение — полная мысль с выводом; никогда не обрывай на союзе, двоеточии или «…». "
            "Без призывов подписаться, без хэштегов, без отдельного блока «Почему это важно»."
        ),
        "newsroom360-growth-brief": (
            "Стиль: Newsroom360 Growth Brief для предпринимателей, инвесторов и финансистов. "
            "Верни JSON с полями: headline, what_happened, why_important, money_impact, what_next. "
            "Каждое поле — 1–2 коротких предложения. money_impact: что это значит для денег, ставок, рынка. "
            "what_next: завершённый вывод без интриги. Без URL, без @username, без кликбейта."
        ),
    }
