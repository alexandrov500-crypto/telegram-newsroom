    ...
    *,
    ...
    blocks.extend(
        [
            ...
            "* единый шаблон как у @cb_economics: headline отдельно (json) + короткий post;",
            "* headline: одна строка — главный факт, без кликбейта и без «…»;",
            "* post: 2–3 абзаца, 2–5 предложений, 350–700 символов;",
            "* структура post: факт → контекст → итоговое предложение с выводом;",
    ...
    blocks.extend(
        [
            ...
            "* до 700 символов в поле post (если задан отдельный заголовок — он не входит в этот лимит);",
...
def _style_block_ru(style: str) -> str:
    m = {
        ...
        "cb-economics-brief": (
            "Стиль: как у эталонного канала @cb_economics — единый формат каждого поста. "
            "headline: одна строка, главный факт (цифра/решение/событие), без интриги и без вопроса. "
            "post: 2–3 коротких абзаца (всего 2–5 предложений, 350–700 символов). "
            "Структура: что произошло → контекст/механизм → краткий итог одним предложением. "
            "Последнее предложение — завершённая мысль (вывод), не «продолжение следует». "
            "Без призывов подписаться, без хэштегов, без блока «Почему это важно» отдельной строкой."
        ),
        "newsroom360-growth-brief": (
